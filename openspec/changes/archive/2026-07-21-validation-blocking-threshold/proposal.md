...
- **(b) validation 类 blocking 的举证门槛**：`src/npc/schema.py::REVIEW_SCHEMA` 的 finding 新增可选字段 `trigger_evidence`（类型 `["string", "null"]`，Round 5 修订：允许显式 `null` 通过 schema，避免其被拒绝而无法到达 `parse_review()` 的降级判定），**不进 `required`、不做任何条件必需**（schema 层面对 `category == "validation"` 的 finding 与其它 category 一视同仁，字段完全可选，缺失该字段不会导致 schema 校验失败）；`src/npc/focus.py::_output_requirements_block()` 新增文案说明该字段的用途与"缺失/空值/占位符会被 `parse_review()` 侧确定性降级"的后果；`src/npc/review.py::parse_review()` 对 `category == "validation"` 的 blocking 候选 finding，若 `trigger_evidence` 缺失键、为 `None`、去除首尾空白后为空字符串或等于占位符 `"-"`，则降级为 advisory（不计入 `blocking`/`blocking_findings`）——这一确定性判断是 `trigger_evidence` 语义生效的唯一入口，不存在与之矛盾的 schema 级强制路径；历史 review 重放与实时生成的 review 共享同一条降级规则，不做实时/历史两套分支。
...
- **verdict 一致性修复**：`parse_review()` 与 `merge_review_passes()`/`_recompute_verdict()` 的 `verdict` 推导统一改为基于降级后的最终 blocking 集合（不再是原始 `severity`/`in_scope`），`parse_review()` 不再透传引擎自报的 `verdict` 值——避免"该轮全部 blocking 候选均被降级"时出现 `blocking == 0` 但 `verdict == "changes-requested"` 的矛盾。本条同步修订 `openspec/specs/review-adversarial-pass/spec.md` 的「findings 合并去重规则确定性」Requirement 第 3 条（合并结果 `verdict` 改为基于降级后集合重算，替代原"任意 high/critical 且 in_scope finding 即 `changes-requested`"表述）与「telemetry 透出对抗通道运行状态」Requirement（显式裁决 `adversarial_blocking_count` 表示 pass2 贡献的原始候选数，MUST NOT 等价于降级后最终 blocking 数，二者在有降级发生时可能不相等）——`merge_review_passes()` 本身已由 `_recompute_verdict()` 复用 `parse_review()` 的降级判定逻辑，此处只是让 `review-adversarial-pass` capability 的既有已归档规范文本与该行为同步，消除两个 capability 对同一输入给出相反结论的矛盾。
...
- `validation-blocking-threshold`：validation 类 blocking 的双向治理——reviewer 侧举证门槛（`trigger_evidence` 可选字段 + `parse_review()` 侧确定性缺失降级）、spec-silent 归因窄化降级（非 critical 降级为 advisory）、降级可观测性计数。

### Modified Capabilities

- `implement-selfcheck-rubric`：`SELFCHECK_RUBRIC_MD` 的 `validation` 类目自查要点细化为四个具体检查点。
- `spec-attribution`：「spec 归因不参与 blocking 判定」「派生 spec 归因分布并向后兼容」「本 change 不引入任何闸门」三条 Requirement 修订——前者窄化并补充 validation 举证门槛降级作为独立例外的显式引用（与 spec-silent 例外并列），为 `validation-blocking-threshold` 能力定义的两条降级规则让出显式例外；中者把统计范围改写为"降级后最终 blocking 集合"；后者确认该例外不构成新增闸门。其余归因值与既有闸门约束不变。
- `review-adversarial-pass`：「findings 合并去重规则确定性」Requirement 第 3 条（`verdict` 重算规则）修订为基于降级后的最终 blocking 集合，替代原"任意 high/critical 且 in_scope finding 即 `changes-requested`"表述；「telemetry 透出对抗通道运行状态」Requirement 修订，显式裁决 `adversarial_blocking_count` 的语义为"pass2 贡献的原始 blocking 候选数（不感知降级）"，与最终 `blocking`/`verdict` 判定解耦并在文本中消歧。

## Impact

- **实现 prompt 渲染**：`src/npc/templates.py`（`SELFCHECK_RUBRIC_MD` 常量文案）。
- **review 举证与降级判定**：`src/npc/schema.py`（`REVIEW_SCHEMA.trigger_evidence` 可选字段，不进 `required`）、`src/npc/focus.py`（`_output_requirements_block()` 新增举证要求文案）、`src/npc/review.py`（`parse_review()` 新增两条降级判据、`downgrade_counts` 返回值、`verdict` 改为基于降级后集合重算而非透传引擎自报值；`merge_review_passes()`/`_recompute_verdict()` 同步改为基于降级后集合判定 `verdict`）。
...
- **spec 归档**：修订 `openspec/specs/spec-attribution/spec.md` 三条既有 Requirement、`openspec/specs/review-adversarial-pass/spec.md` 两条既有 Requirement（均作为本 change 的 MODIFIED Requirements delta）。
