...
用户在盘问阶段已就三个关键分歧点拍板（见下方 Pattern Mapping），核心结论：(c) 显式修订 `spec-attribution` capability 的两条既有 Requirement（窄化范围）；(b) 采用 schema 条件必需字段 + npc 侧确定性降级（而非纯 prompt 判据）；(c) 的降级例外是 `severity == critical` 不降级；本轮就新增降级相关的 telemetry 计数。
...
- `REVIEW_SCHEMA` 对 `category == "validation"` 的 finding 条件必需 `trigger_evidence` 字段（key 必须存在，值是否非空由 npc 侧判断）；`parse_review()` 对该字段缺失/为空/为占位符 `"-"` 的 validation blocking 候选 finding 确定性降级为 advisory。
...
- 不新增失败原因码、退出码或下游 gate；`downgrade_counts` 仅用于可观测性，不驱动任何阻断/阈值/`auto-decide` 触发条件。（`REVIEW_SCHEMA` 新增的 `trigger_evidence` 条件必需字段确实新增了一种可触发既有 `invalid_review_schema` 重试-失败路径的 schema 违例条件——这不违反本 Non-Goal：该条件复用的是既有失败原因码 `invalid_review_schema`、既有重试-失败机制与既有下游 gate，未新增任何一种，只是新增了一种"命中既有路径"的触发方式，与 `REVIEW_SCHEMA` 其它必填字段校验失败时复用的是同一条路径。见 `validation-blocking-threshold/spec.md`「validation 类 finding 携带条件必需的触发证据字段」Requirement 与 Migration Plan 第 1 条。）
...
- Q2 → 见 Decisions D2：`REVIEW_SCHEMA` 新增 `trigger_evidence` 条件必需字段（`if/then`，仅约束 key 存在），`parse_review()` 对空值/占位符做确定性降级。
...
**D2：`REVIEW_SCHEMA` 用 `allOf`/`if`/`then` 对 `category == "validation"` 的 finding 条件必需 `trigger_evidence`；该约束只保证 key 存在，不保证非空。**

`REVIEW_SCHEMA["properties"]["findings"]["items"]` 新增：

- `properties.trigger_evidence`：`{"type": "string", "description": "category == validation 时必须描述可复现该问题的具体输入值 / 请求体 / 调用序列 / 参数组合；非 validation 类可留空或省略此字段的实质内容"}`。
- `additionalProperties` 保持 `false`（既有约束不变），因此该属性必须声明在 `properties` 中才能被任意 finding 携带（包括非 validation 类想选填的场景）。
- 顶层新增 `"allOf": [{"if": {"properties": {"category": {"const": "validation"}}, "required": ["category"]}, "then": {"required": ["trigger_evidence"]}}]`（放在 `items` 对象内，与 `properties`/`required`/`additionalProperties` 同级）。`category != "validation"` 时不要求该字段。
- **不使用** `minLength: 1`：Schema 层面只保证字段存在（key present），不保证非空——空字符串仍能通过 schema 校验。这是有意设计（对齐 Q2 裁决"缺失该字段时由 npc 侧确定性降级"）：若 schema 直接拒绝空值，会导致 codex 引擎产出不合法 JSON 触发重试/失败路径，而不是本 change 想要的"照常出具 review，但该 finding 被 npc 侧降级"路径。
...
- `merge_review_passes()` 的 `adversarial_blocking_count`（side-channel 统计，仅用于 telemetry 观测 pass2 独有贡献的 blocking 数）保持原有"按原始 severity/in_scope 计数、不感知降级"的既有语义不变——这是一个独立的可观测性统计，不参与 `verdict`/`blocking` 判定，不属于本 Decision 的修改范围。
...
1. `REVIEW_SCHEMA` 新增 `trigger_evidence` 属性与 `allOf` 条件必需规则会被 `ensure_schema`（内容比对触发重写）自动同步到 `~/task_log/.new-plan-review-schema.json`，无需手工迁移；引擎持续产出缺失该字段的 validation finding 时，既有 `_execute_review_pass()` 重试-失败机制（`invalid_review_schema`）照常生效，不是新的失败路径（见「validation 类 finding 携带条件必需的触发证据字段」Requirement 的 schema 失败流程 Scenario）。
2. 历史 `round-N.review.json`（本 change 落地前生成，不含 `trigger_evidence` 字段）不受影响：`parse_review()` 是纯函数，不做 schema 校验，`_validation_evidence_missing` 对缺失字段的处理路径（`ev is None → True`）与"新生成但值为空"的处理路径完全一致，历史文件的 validation 类 blocking finding 在重新解析时会被判定为"证据缺失"从而降级——这是预期行为（该字段本就未提供，降级符合"缺失举证即降级"的规则本身），不需要额外的兼容分支。这类历史文件重放也是「validation 类 finding 缺失触发证据时降级为 advisory」Requirement 中"键不存在"/`null` 两个判定分支在实践中唯一可达的场景（实时 schema 校验通过的当前流程不会产出这两种状态）。
