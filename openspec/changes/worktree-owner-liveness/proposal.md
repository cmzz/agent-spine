...
- **新增 owner 存活判定模块** `src/npc/owner.py`：`capture_owner_fields()`（写入侧，供 `state.py`/`init_cmd.py` 复用同一份"`os.getppid()` 父进程 pid + 心跳时间戳"快照逻辑；`owner_pid` 记录的是触发本次子命令调用的父进程——CC 主 session / shell，而非 npc 子进程自身）与 `owner_alive(state: dict) -> bool`（判定侧：`os.kill(pid, 0)` 探测**只能确认存活、不能确认死亡**——`owner_pid` 在真实部署中常是短命包装 shell，pid 死亡不构成 owner 死亡证据；判死完全由 24 小时心跳新鲜度决定，详见 design.md review round 2 修订）。
...
- **新增 `npc init --takeover`**（review round 2）：显式接管 owner 判定仍存活的 in-progress / initializing 候选，覆盖"真崩溃但心跳尚未过期、用户确认原 session 已死"的立即恢复场景；与 `--fresh` 互斥；不豁免孤儿标记的 owner 门槛。
