...
# ============================================================
# round≥2 delta-review 收敛（change review-delta-convergence）
# ============================================================


def _pf(id_, sev, cat, file, line, origin, in_scope=True):
    return {
...
def _round_n_setup(p, base_phases: dict) -> Path:
    """seed 好 seq=1/add-foo 的 phases 并返回 base 目录（假设 _bootstrap_run 已跑过）。"""
    base = Path(json.loads(p.state_json.read_text())["progress"][0]["base"])
    base.mkdir(parents=True, exist_ok=True)
    _seed_phases(p, 1, base_phases)
    return base


def _run_round_n(p, fake_repo, monkeypatch, round_n, review_payload):
    monkeypatch.setattr(_pipeline, "_codex_exec", _stub_codex_writes_review(review_payload))
    monkeypatch.setattr(_pipeline, "_find_codex_bin", lambda override=None: "/fake/codex")
    monkeypatch.setattr(
        _pipeline, "_portable_timeout_bin", lambda override=None: Path("/fake/portable-timeout")
    ...
    p_with_repo = type(p)(**{**p.__dict__, "repo_root": fake_repo})
    return _pipeline.run_review_round(p_with_repo, 1, round_n)


def test_run_review_round2_render_focus_receives_round_fix_commit(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    ...
    result = _run_round_n(
        p, fake_repo, monkeypatch, 2, {"verdict": "approve", "findings": []}
    )
    assert result["ok"] is True
    focus_text = (base / "round-2.focus.md").read_text(encoding="utf-8")
    assert "git --no-pager diff fixc0mmit1~1..fixc0mmit1" in focus_text


def test_run_review_round2_prior_blocking_from_default_parse_of_round1(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.4：round_n=2 时 prior_blocking 取自对 round-1.review.json 的默认 parse_review。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    base = _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
    ...
    # round-1.review.json 是历史文件，不含 finding_origin（也应能被默认 parse_review 消费）
    round1_finding = {
    ...
    (base / "round-1.review.json").write_text(
        json.dumps({"verdict": "changes-requested", "findings": [round1_finding]}),
        encoding="utf-8",
    )
    review_payload = {
        "verdict": "changes-requested",
        "findings": [_pf("F1", "high", "validation", "a.py", "10-20", "pre-existing-new")],
    }
...
def test_run_review_round3_prior_blocking_from_state_not_reparsed_round2_json(
    ...
    """task 4.4b：round3 的 prior_blocking 取自 state 持久化的 effective_blocking_findings，
    round2 曾把某三元组降级为 pre-existing-new（不在其中），round3 再报同一三元组
    不应被误判为 carry-over-unresolved（对应 spec Scenario：降级后再次出现不被误判）。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    base = _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
        "fix-r1": {"status": "done", "commit": "fixc0mmit1"},
        "fix-r2": {"status": "done", "commit": "fixc0mmit2"},
        "review-r2": {
            "status": "done",
            "blocking": 0,
            "carryover_unresolved_blocking": 0,
            "hard_convergence_applied": False,
            "effective_blocking_findings": [],  # T 已被降级，不在其中
        },
    })
    review_payload = {
        "verdict": "changes-requested",
        "findings": [_pf("F1", "high", "validation", "z.py", "5-6", "pre-existing-new")],
    }
    result = _run_round_n(p, fake_repo, monkeypatch, 3, review_payload)
    assert result["ok"] is True
    assert result["blocking"] == 0
    assert result["carryover_unresolved_blocking"] == 0


def test_run_review_round3_missing_effective_blocking_findings_disables_delta(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.4c：上一轮缺 effective_blocking_findings 字段 → prior_blocking=None →
    完全禁用几何核验，信任自报 verdict/severity/in_scope，carryover 为 None。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
        "fix-r1": {"status": "done", "commit": "fixc0mmit1"},
        "fix-r2": {"status": "done", "commit": "fixc0mmit2"},
        "review-r2": {"status": "done", "blocking": 0, "carryover_unresolved_blocking": 0},
    })
    review_payload = {
        "verdict": "changes-requested",
        "findings": [_pf("F1", "high", "validation", "a.py", "1-2", "carry-over-unresolved")],
    }
    result = _run_round_n(p, fake_repo, monkeypatch, 3, review_payload)
    assert result["ok"] is True
    assert result["verdict"] == "changes-requested"  # 直接信任自报值
    assert result["blocking"] == 1
    assert result["carryover_unresolved_blocking"] is None
    assert result["hard_convergence_applied"] is False


def test_run_review_round3_hard_convergence_overrides_to_passed_with_advisory(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.5：连续两轮无遗留、round3 有一条 round-diff-new blocking → 覆盖为
    blocking=0/passed-with-advisory，findings 转记 advisory。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    base = _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
    ...
    review_payload = {
        "verdict": "changes-requested",
        "findings": [_pf("F1", "critical", "validation", "a.py", "1-2", "round-diff-new")],
    }
    result = _run_round_n(p, fake_repo, monkeypatch, 3, review_payload)
    assert result["ok"] is True
    assert result["blocking"] == 0
    assert result["hard_convergence_applied"] is True
    assert result["verdict"] == "passed-with-advisory"
    assert result["advisory"] == 1
    # 硬收敛覆盖后不渲染下一轮 fix findings
    assert result["findings_path"] is None
    assert not (base / "round-4.fix.findings.md").exists()
    # advisory carryover 清单包含被降级的原 blocking finding
    advisory_md = base / "round-3.advisory-carryover.md"
    assert advisory_md.is_file()
    assert "F1" in advisory_md.read_text(encoding="utf-8")


def test_run_review_round3_hard_convergence_overrides_to_approve_when_empty(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.5b：覆盖前无任何 blocking/advisory finding → verdict=approve（非硬编码）。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
    ...
    review_payload = {"verdict": "changes-requested", "findings": []}
    result = _run_round_n(p, fake_repo, monkeypatch, 3, review_payload)
    assert result["ok"] is True
    assert result["blocking"] == 0
...
def test_run_review_round3_no_hard_convergence_when_prior_round_has_carryover(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.6：round2 遗留非零 → 不触发覆盖。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
        ...
        "fix-r2": {"status": "done", "commit": "fixc0mmit2"},
        "review-r2": {
            "status": "done", "blocking": 2,
            "carryover_unresolved_blocking": 2,
            "effective_blocking_findings": [
                {"file": "a.py", "line_range": "1-2", "category": "validation"},
                {"file": "b.py", "line_range": "3-4", "category": "validation"},
            ],
        },
    })
    review_payload = {"verdict": "changes-requested", "findings": []}
...
def test_run_review_round2_hard_convergence_not_applicable_below_round3(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.7：round_n < 3 时即使本轮 carryover=0 也不触发硬收敛覆盖。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    base = _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
...
def test_run_review_round2_advisory_carryover_written_with_pre_existing_finding(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.9：round≥2 落盘 advisory-carryover.md，含 pre-existing-new finding。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    base = _round_n_setup(p, {
        "implement": {"status": "done", "commit": "deadbeef"},
    ...
    review_payload = {
        "verdict": "changes-requested",
        "findings": [_pf("F1", "high", "validation", "z.py", "9-9", "pre-existing-new")],
    }
    result = _run_round_n(p, fake_repo, monkeypatch, 2, review_payload)
    assert result["ok"] is True
    advisory_md = base / "round-2.advisory-carryover.md"
    assert advisory_md.is_file()
    assert "F1" in advisory_md.read_text(encoding="utf-8")


def test_run_review_round0_and_round1_do_not_produce_advisory_carryover(
    env_setup, make_args, capsys, monkeypatch, fake_repo: Path
):
    """task 4.10：round_n < 2 不产出 advisory-carryover.md。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    review_payload = {"verdict": "approve", "findings": []}
    result0 = _run_round_n(p, fake_repo, monkeypatch, 0, review_payload)
    assert result0["ok"] is True
    base = Path(result0["review_json"]).parent
    assert not (base / "round-0.advisory-carryover.md").exists()

    _set_implement_commit(p.state_json, "deadbeef")
    result1 = _run_round_n(p, fake_repo, monkeypatch, 1, review_payload)
    assert result1["ok"] is True
    assert not (base / "round-1.advisory-carryover.md").exists()
...
# ============================================================
# _do_review_phase_exit_and_trend 新字段（change review-delta-convergence，task 4.1/4.2）
# ============================================================


def test_review_phase_exit_default_round_n_writes_none_false_none(
    env_setup, make_args, capsys, monkeypatch
):
    """round_n 缺省（<2）：三新键分别写 None/False/None。"""
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    _capture_recurrence(monkeypatch)
    _pipeline._do_review_phase_exit_and_trend(
        p, 1, "review-r1", {"blocking": 1, "categories": [], "verdict": "x"}
    )
    state = _state.read_state(p.state_json)
    phase = state["progress"][0]["phases"]["review-r1"]
    assert phase["carryover_unresolved_blocking"] is None
    assert phase["hard_convergence_applied"] is False
    assert phase["effective_blocking_findings"] is None


def test_review_phase_exit_round_n2_writes_effective_blocking_findings_triples(
    env_setup, make_args, capsys, monkeypatch
):
    p = env_setup
    _bootstrap_run(env_setup, make_args, capsys, "add-foo")
    ...
    metrics = {
        ...
        "verdict": "changes-requested",
        "carryover_unresolved_blocking": 1,
        "hard_convergence_applied": False,
        "blocking_findings": [
            {"id": "F1", "file": "a.py", "line_range": "1-2", "category": "validation", "extra": "x"}
        ],
    }
    _pipeline._do_review_phase_exit_and_trend(p, 1, "review-r2", metrics, round_n=2)
    state = _state.read_state(p.state_json)
    phase = state["progress"][0]["phases"]["review-r2"]
    assert phase["carryover_unresolved_blocking"] == 1
    assert phase["hard_convergence_applied"] is False
    assert phase["effective_blocking_findings"] == [
        {"file": "a.py", "line_range": "1-2", "category": "validation"}
    ]
