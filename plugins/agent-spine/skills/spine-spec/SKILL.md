...
- Invoke every `npc spec review run` with `--engine claude`. A Claude dependency or execution failure stops the review; Kimi MUST NOT review its own spec as a fallback.
