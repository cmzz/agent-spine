...
### 11.7 auto 档破坏性操作 deny 底线

**背景**：`npc init --auto` 已通过 `settings.json` 置 `defaultMode=acceptEdits` + harness Bash 白名单（放宽 allow），以及 `settings.local.json` 置 `additionalDirectories`（受信 cwd 外目录）。但放宽「允许」同时缺乏「禁止」底线——长 run 中 context compaction 后 prompt 层约束会衰减，而 `git push --force`、`git reset --hard`、直改 `.git/` 内部在权限层无阻拦。

**方案**：`grant_auto_local_dirs`（写 `settings.local.json`）在写入 `additionalDirectories` 的同时，追加 `permissions.deny` 底线：

| deny 条目 | 防护目标 |
|---|---|
| `Bash(git push --force*)` | 禁止强制推送覆盖远端历史 |
| `Bash(git reset --hard*)` | 禁止硬重置丢弃本地 commits |
| `Edit(.git/**)` | 禁止直接修改 git 内部文件 |

**关键性质**：deny 规则属 Claude Code permissions 系统（settings 层），在**每次工具调用时强制**，不依赖 context，因此 compaction 后仍恒定生效。这是 prompt 层约束（system prompt / 指令）做不到的。

**合并语义**：与用户既有 `deny` 取并集追加（不删不改用户条目）、幂等（重复 init 不重复追加）、坏 JSON 不覆盖、写失败不阻塞 init，与 `grant_auto_permissions` 保持同纪律。
